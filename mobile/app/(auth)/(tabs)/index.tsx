...
import {
  Bell, Book, BookOpen, Edit2, Layers, LogOut, MessageSquare,
  Mic2, Moon, PenTool, Search, Sun, X
...
const SOCKET_URL = 'http://192.168.2.6:5000'; // ← ĐỔI THÀNH IP CỦA BẠN
        ...
        <ScrollView contentContainerStyle={styles.contentFullWidth}>
          ...
          <View style={styles.quickActionsGrid}>
            {QUICK_ACTIONS.map((action, index) => (
              <TouchableOpacity key={index} style={[styles.actionBtnV2, { backgroundColor: action.bg, shadowColor: action.color }]}
                onPress={() => {
                  if (action.title === 'Sách Song Ngữ') router.push('/books/list' as any);
                  else if (action.title === 'Flashcard') router.push('/FlashSet' as any);
                  else if (action.title === 'Luyện Thi') router.push('/(quiz)' as any);
                  else if (action.title === 'Shadowing') router.push('/(shadowing)' as any);
                }}>
