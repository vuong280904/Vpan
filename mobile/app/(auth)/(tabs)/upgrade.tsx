...
// import { ArrowLeft,  Check, Crown, Sparkles, Zap } from 'lucide-react-native';
...
export default function UpgradeScreen() {
    ...
    const createPayment = async (plan: Plan) => {
        ...
        setInvoiceId(reference);

        try {
            const randomBytes = await Crypto.getRandomBytesAsync(32);
            const ref = new PublicKey(randomBytes);

            // ✅ STRING – KHÔNG BigNumber
            const amountInSol = IS_TEST
            ? '0.01' // 🔥 chỉ 0.01 SOL
            : (plan.price / 100).toFixed(3);
            ...
            const url = buildSolanaPayURL({
                ...
                reference: ref,
                label: 'Vpan Upgrade',
                message: `${plan.name} - ${user?.email}`,
            });

            const qr =
                'https://api.qrserver.com/v1/create-qr-code/?size=340x340&data=' +
                encodeURIComponent('9rZttxsDzghUFkuZ7FxYYnSFzi2TcYU46yr5pStfJr2m');

            setSelectedPlan(plan);
            setQrCode(qr);
            setPayAmount(amountInSol);
            setReference(ref.toBase58());
            setPaymentModal(true);
        } catch {
            Alert.alert('Lỗi', 'Không thể tạo QR Solana Pay');
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
    if (!paymentModal || !reference) return;
